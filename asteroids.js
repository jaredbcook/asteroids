(function() {
	...
	var pointInPolygon = function(point, objectGeometry) {
		var i, j = objectGeometry.length - 1;
		var oddNodes = false;

		for (i = 0; i < objectGeometry.length; i++) {
			if ((objectGeometry[i].y < point.y && objectGeometry[j].y >= point.y || objectGeometry[j].y < point.y && objectGeometry[i].y >= point.y) && (objectGeometry[i].x <= point.x || objectGeometry[j].x <= point.x)) {
				oddNodes ^= (objectGeometry[i].x + (point.y - objectGeometry[i].y) / (objectGeometry[j].y - objectGeometry[i].y) * (objectGeometry[j].x - objectGeometry[i].x) < point.x);
			}
			j = i;
		}
	...
	GameWorldObject.prototype.destroy = function() {
		var i, gameWorldObjects = GameWorld.getObjects();
	...
	GameWorldObject.prototype.render = function() {
		...
		if (this.geometry && this.geometry.length === 1) {
			...
			if (this.textureImg) {
				context.clip();
				context.drawImage(this.textureImg, this.position.x - (2 * this.radius), this.position.y - (2 * this.radius), this.textureImg.width, this.textureImg.height);
			}
		}
		context.stroke();
		context.restore();
	...
	var Ship = function(position, velocity, angle, radius) {
		...
		// ship settings
		this.acceleration = .4;	// amount by which to increment the directional velocity
		this.rotationSpeed = 5;	// number of degrees to rotate the ship per time cycle
		this.friction = .008;	// friction only applies to the ship
		this.fireRate = .2;		// rate at which the ship is allowed to fire bullets (in seconds)
		...
		this.geometry.push(new Vector(this.position.x + this.radius, this.position.y));							// front tip
		this.geometry.push(new Vector(this.position.x - this.radius, this.position.y + (this.radius * 3 / 4)));	// right wing tip
		this.geometry.push(new Vector(this.position.x - (this.radius / 2), this.position.y));					// rear indentation point
		this.geometry.push(new Vector(this.position.x - this.radius, this.position.y - (this.radius * 3 / 4)));	// left wing tip
	};
	...
	Ship.prototype.accelerate = function(angle) {
		...
		if (Math.sqrt(Math.pow(this.velocity.x, 2) + Math.pow(this.velocity.y, 2)) < this.maxVelocity) {
			this.velocity.x += Math.cos(2 * Math.PI * (angle / 360)) * this.acceleration;
			this.velocity.y -= Math.sin(2 * Math.PI * (angle / 360)) * this.acceleration;
		}
	};
	...
	Ship.prototype.fire = function() {
		...
		if (this.lastFired && Date.now() - this.lastFired > this.fireRate * 1000) {
			GameWorld.getObjects().push(new Bullet(new Vector(this.position.x, this.position.y), this.angle));
	...
	Ship.prototype.update = function() {
		var i, j, asteroid, collision = false, dx, dy, distanceApart;
		...
		for (i = 0; i < gameWorldObjects.length; i++) {
			if (gameWorldObjects[i] instanceof Asteroid) {
				...
				if (distanceApart < (this.radius * .9) + (asteroid.radius * .9)) {
					collision = true;
				}
	...
	Bullet.prototype.update = function() {
		var i, asteroid, collision = false, dx, dy, distanceApart;
		var gameWorldObjects = GameWorld.getObjects();
	...
	var Asteroid = function(position, velocity, angle, radius) {
		...
		for (ang = 0; ang < 2 * Math.PI; ang += 2 * Math.PI / granularity) {
			...
			this.geometry.push(new Vector(this.position.x + x, this.position.y + y));
		}
	};
	...
	Asteroid.prototype.explode = function() {
		...
		if (this.radius >= AsteroidRadius.MEDIUM) {
			...
			for (i = 0; i < 3; i++) {
				vx = Math.random() * AsteroidMaxVelocity.MEDIUM - (AsteroidMaxVelocity.MEDIUM / 2);
				vy = Math.random() * AsteroidMaxVelocity.MEDIUM - (AsteroidMaxVelocity.MEDIUM / 2);

				// add asteroid to game world
				GameWorld.getObjects().push(new Asteroid(
			...
			for (i = 0; i < 3; i++) {
				vx = Math.random() * AsteroidMaxVelocity.SMALL - (AsteroidMaxVelocity.SMALL / 2);
				vy = Math.random() * AsteroidMaxVelocity.SMALL - (AsteroidMaxVelocity.SMALL / 2);
	...
	var GameWorld = (function() {
			...
			for (i = 0; i < numAsteroids; i++) {
				...
				while (placementTooClose && numRetries < 100) {
					...
					for (j = 0; j < gameWorldObjects.length; j++) {
						...
						if (Math.sqrt(Math.pow(dx, 2) + Math.pow(dy, 2)) < AsteroidRadius.LARGE * 4) {
							placementTooClose = true;
							numRetries++;
							break;
						}
				...
				// generate velocity
				vx = Math.random() * AsteroidMaxVelocity.LARGE - (AsteroidMaxVelocity.LARGE / 2);
				vy = Math.random() * AsteroidMaxVelocity.LARGE - (AsteroidMaxVelocity.LARGE / 2);

				// add asteroid to game world
				gameWorldObjects.push(new Asteroid(
		...
		var playLevelReadyScene = function() {
			var levelTitleNode = levelReadySceneNode.getElementsByClassName('scene-title')[0];
